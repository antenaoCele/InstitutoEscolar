...
import { validateID, checkValidations, validateSchedules } from "./validations.js";
...
router.get("/", authentication, async (req, res) => {
  const [rows] = await db.execute(`
    SELECT s.*, t.name AS teacher, sub.name AS subject
    FROM schedules s
...
router.post("/",
  ...
  async (req, res) => {
    ...
    res.status(201).json({
      success: true,
      data: { id: result.insertId, teacher_id, subject_id, start_time, end_time, days }
    });
});
...
router.put("/:id",
  ...
  async (req, res) => {
    ...
    const [exists] = await db.execute("SELECT id FROM schedules WHERE id=?", [id]);
    if (exists.length === 0) {
      return res.status(404).json({ success: false, error: "Horario no encontrado" });
    }
    ...
    res.json({ success: true, data: { id, teacher_id, subject_id, start_time, end_time, days } });
});
