...
import { validateID, checkValidations, validatePayments } from "./validations.js";
...
router.get("/:id",
  ...
  async (req, res) => {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ success: false, error: "Pago no registrado" });
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    const [result] = await db.execute(
      "INSERT INTO payments (student_plan_id, amount, payment_method) VALUES (?,?,?)",
      [student_plan_id, amount, payment_method]
    );

    res.status(201).json({
      success: true,
      data: { id: result.insertId, student_plan_id, amount, payment_method },
    });
});
...
router.put("/:id",
  ...
  async (req, res) => {
    ...
    const [payment] = await db.execute("SELECT id FROM payments WHERE id=?", [id]);
